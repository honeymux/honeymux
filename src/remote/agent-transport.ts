...
export interface RemoteAgentIngress {
  /** Shared secret embedded in the tmux user-option; remote hook events must carry it. */
  readonly authToken: string;
  close(): void;
  /**
   * Loopback TCP port the local agent ingress is bound to. Meaningful only after
   * `start()`. `0` until the listener has bound.
   */
