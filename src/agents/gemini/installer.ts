 ...
 * `installGeminiHooks` would produce — i.e. running sync would be a no-op.
 * Used to suppress the "upgrade" prompt when nothing is actually stale.
 */
export async function isGeminiHookInstallCurrent(host: InstallHost = localInstallHost): Promise<boolean> {
  const scriptPath = join(await getHooksDir(host), HOOK_SCRIPT_NAME);
  ...
  const nextSettingsText = JSON.stringify(
    upsertGeminiHookSettings(safeParseJson(currentSettingsText), command),
    null,
    2,
  );
