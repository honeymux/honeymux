...
describe("TmuxControlClient connection guards", () => {
  test("keeps generic and session-scoped window selection explicit", async () => {
    ...
    await client.selectWindowInSession("beta renamed", "@3");

    expect(sendCommand).toHaveBeenNthCalledWith(1, `select-window -t ${quoteTmuxArg("windowId", "@3")}`);
    expect(sendCommand).toHaveBeenNthCalledWith(
      2,
      `select-window -t ${quoteTmuxArg("windowTarget", "beta renamed:@3")}`,
    );
  });
